...
const view_basicDraftParameters = view<{
...
app({
    ...
    run: async (run, ui) => {
        ...
        if (ui.promptFromLlm.runLLM.RunSD) {
            ...
            // run.output_text({ title: 'workflow', message: 'test' + JSON.stringify(returnedPrompt.graph) })
            run.output_custom({
            ...
            const returnedPrompt = await run.PROMPT({ saveFormat })
