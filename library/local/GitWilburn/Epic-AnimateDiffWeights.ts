...
export const ui_vhsExport = () => {
    ...
    return form.fields({
        filename_prefix: form.string({ default: undefined }),
        format: form.enum.Enum_VHS$_VideoCombine_format({ default: undefined }),
        frame_rate: form.float({ default: undefined, min: 1, step: 1 }),
        advancedSettings: form.fields({
            loop_count: form.int({ default: undefined, min: 0, max: 100, step: 1 }),
            pingpong: form.boolean({ default: undefined }),
            save_output: form.boolean({ default: undefined }),
...
app({
    ...
    ui: (form) => ({
        ...
        AnimateDiffSettings: form.fields({
            ...
            context_overlap: form.int({ default: 4, min: 0, max: 128 }),
            AdvancedSettings: form.fields(
                {
        ...
        IPAdapterSettings: form.fields({
            ...
            framesPerImage: form.int({ default: 6, min: 0, max: 9999, step: 1 }),
            add_starting_frames: form.int({ default: 8, min: 0, max: 9999, step: 1 }),
            add_ending_frames: form.int({ default: 8, min: 0, max: 9999, step: 1 }),
            ...
            AdvancedSettings: form.fields({
                ...
                start_frame: form.int({ default: 0, min: 0, max: 9999, step: 1 }),
                end_frame: form.int({ default: 9999, min: 0, max: 9999, step: 1 }),
        ...
        SecondPass: form.fields({
            sampler: ui_sampler(),
            control_net_name: form.enum.Enum_ControlNetLoader_control_net_name({ default: 'animate_controlnet_checkpoint.ckpt' }),
            strength: form.float({ default: 0.6, min: 0, max: 10, step: 0.01 }),
            start_percent: form.float({ default: 0, min: 0, max: 1, step: 0.001 }),
            end_percent: form.float({ default: 0.6, min: 0, max: 1, step: 0.001 }),
            IPAdapter: form.fields({
                weight: form.float({ default: 0.35000000000000003, min: -1, max: 5, step: 0.05 }),
                weight_type: form.enum.Enum_IPAdapterAdvanced_weight_type({ default: 'linear' }),
                start_at: form.float({ default: 0, min: 0, max: 1, step: 0.001 }),
                end_at: form.float({ default: 1, min: 0, max: 1, step: 0.001 }),
                embeds_scaling: form.enum.Enum_IPAdapterAdvanced_embeds_scaling({ default: 'V only' }),
    ...
    run: async (run, ui) => {
        ...
        let { latent, width, height } = await run_latent_v3({
            opts: ui.latent,
            vae,
            width_override: ui.latent.emptyLatent?.size.width,
            height_override: ui.latent.emptyLatent?.size.height,
        })
        ...
        const iPAdapterWeights = graph.IPAdapterWeights({
            weights: ui.IPAdapterSettings.weights,
        ...
        const aCN$_AdvancedControlNetApply = graph.ACN$_AdvancedControlNetApply({
