...
// 🅿️ SoftEdge FORM ===================================================
export const ui_subform_SoftEdge = (form: FormBuilder) => {
    return form.group({
        label: 'SoftEdge',
        items: () => ({
            ...cnet_ui_common(form),
            ...
            cnet_model_name: form.enum({
                ...
                default: 'control_v11p_sd15_softedge.pth',
                group: 'Controlnet',
                label: 'Model',
            }),
        }),
...
export const run_cnet_SoftEdge = async (flow: Runtime, SoftEdge: OutputFor<typeof ui_subform_SoftEdge>, cnet_args: Cnet_args) => {
    ...
    image = graph.ImageScale({
        image: (await flow.loadImageAnswer(SoftEdge.image))._IMAGE,
    ...
    if (SoftEdge.preprocessor?.type.HED) {
        ...
        if (hed.saveProcessedImage)
            graph.SaveImage({ images: image, filename_prefix: 'cnet\\SoftEdge\\hed' })
        else
        ...
        if (pid.saveProcessedImage)
            graph.SaveImage({ images: image, filename_prefix: 'cnet\\SoftEdge\\pid' })
        else
            graph.PreviewImage({ images: image })
    }
